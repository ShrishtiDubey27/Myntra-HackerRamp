// Stop voice recording handler
  ...
  const Waveform = () => (
    <div className="flex gap-1 items-center h-6">
      {[1,2,3,4,5].map(i => (
        <div key={i} className="w-1 h-full bg-blue-400 animate-pulse" style={{ animationDelay: `${i * 0.15}s` }} />
      ))}
    </div>
  );
...
const ChatAI = () => {
  ...
  const sendMessage = async (e) => {
    ...
    try {
      ...
      console.error(error);
      addMessage({ sender: 'bot', text: 'Oops! Something went wrong.', fallback: true, time: new Date() });
    }
  };
  ...
  return (
    <div className="flex flex-col h-[70vh] w-[80%] mx-auto mt-6 bg-white rounded-xl border border-gray-200 shadow-[0_12px_40px_rgba(0,0,0,0.6)] overflow-hidden">
      ...
      <div className="flex justify-between items-center p-4 bg-gradient-to-r from-orange-500 to-orange-600 text-white font-bold text-xl rounded-t-xl">
        ...
        <button onClick={handleClearHistory} className="text-xs font-normal bg-white/20 hover:bg-white/30 px-2 py-1 rounded">
          Clear History
        </button>
      </div>
      ...
      <form className="flex p-4 bg-white border-t border-gray-300 sticky bottom-0" onSubmit={sendMessage}>
        ...
        <div className="flex gap-2 items-center">
          <button type="submit" className="px-4 py-2 bg-orange-500 text-white rounded-full hover:bg-orange-600 transition">Send</button>
          {isRecording ? (
            <div className="flex items-center gap-2">
              <button type="button" disabled className="px-4 py-2 bg-red-500 text-white rounded-full animate-pulse cursor-not-allowed">🎙️ Recording...</button>
              <Waveform />
              <button type="button" onClick={handleStopRecording} className="px-4 py-2 bg-gray-700 text-white rounded-full hover:bg-gray-900 transition">⏹️ Stop</button>
            </div>
          ) : (
            <button type="button" onClick={handleVoiceInput} className="px-4 py-2 bg-blue-500 text-white rounded-full hover:bg-blue-600 transition">🎤 Voice</button>
          )}
          <button type="button" onClick={handleClearHistory} className="px-4 py-2 bg-gray-300 text-gray-800 rounded-full hover:bg-gray-400 transition">Clear</button>
          {isSpeaking && (
            <button type="button" onClick={handlePauseSpeaking} className="px-4 py-2 bg-red-400 text-white rounded-full hover:bg-red-500 transition">⏸️ Pause</button>
          )}
        </div>
      </form>
      {isSpeaking && (
        <div style={{ color: '#007bff', marginTop: '8px' }}>🔊 AI is speaking...</div>
      )}
    </div>
  );
};
