...
const pollSchema = new mongoose.Schema({
  ...
  pollType: {
    type: String,
    enum: ["fashion_trend", "outfit_choice", "color_preference", "style_rating", "general"],
    default: "fashion_trend",
  },
  category: {
    type: String,
    enum: ["dress", "shoes", "accessories", "tops", "bottoms", "formal", "casual", "party", "general"],
    default: "general",
  },
  allowMultipleVotes: {
    type: Boolean,
...
pollSchema.methods.addVote = function(userId, optionId) {
  ...
  console.log('Available options:', this.options.map(opt => ({ id: opt.id, text: opt.text })));
  ...
  if (!this.allowMultipleVotes) {
    ...
    const hasVotedOnOption = option.votes.some(vote => 
  ...
  // Update total votes
  this.totalVotes = this.options.reduce((total, opt) => total + opt.voteCount, 0);
...
pollSchema.methods.getResults = function() {
  return this.options.map(option => ({
    ...
    percentage: this.totalVotes > 0 ? Math.round((option.voteCount / this.totalVotes) * 100) : 0,
  }));
};
